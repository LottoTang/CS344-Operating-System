/*
    ...
    Date:           11/30/2022
    File:           enc_server.c
    Description:    connect with enc_client.c to perform otp encryption
...
void receiveContent(int socketFD, char* outputBuffer, int textLength) {
...
void otpEncryption(char *plaintext, char *key, char *encryptedText, int plaintextLength) {
  ...
  for (int i = 0; i < plaintextLength; i++) {
    ...
    encryptNumber = (plaintextNumber + keyNumber) % 27;

    if (encryptNumber == 26) {
      encryptedText[i] = ' ';
    } else {
      encryptedText[i] = 'A' + encryptNumber;
    }  
    // testing - passed
    //printf("%d: %d: %c\n", i, encryptNumber, encryptedText[i]);
      ...
      case 0:
        {
          ...
          char *plaintextBuffer = calloc(plaintextLength, sizeof(char));
          receiveContent(connectionSocket, plaintextBuffer, plaintextLength);
          ...
          // encryptedText should fill with the encrypted key after the function
          otpEncryption(plaintextBuffer, keytextBuffer, encryptedText, plaintextLength);
